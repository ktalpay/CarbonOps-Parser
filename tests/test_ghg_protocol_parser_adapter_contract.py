...
def test_ghg_parser_adapter_descriptor_is_exact() -> None:
    descriptor = describe_ghg_protocol_parser_adapter()

    assert descriptor == GHGProtocolParserAdapterDescriptor(
...
def test_ghg_parser_adapter_supports_only_ghg_protocol() -> None:
    descriptor = describe_ghg_protocol_parser_adapter()

    assert descriptor.source_family == GHG_PROTOCOL_SOURCE_FAMILY
    assert descriptor.capability.source_family == GHG_PROTOCOL_SOURCE_FAMILY
    assert descriptor.source_family == "ghg_protocol"
    assert descriptor.source_family not in {"defra_desnz", "ipcc_efdb"}


def test_ghg_parser_key_matches_selection_registry() -> None:
    descriptor = describe_ghg_protocol_parser_adapter()
    registry = create_phase1_parser_selection_registry()
    registry_identity = next(
        identity
        for identity in registry.identities
        if identity.source_family == GHG_PROTOCOL_SOURCE_FAMILY
    )

    assert GHG_PROTOCOL_PARSER_KEY == PHASE1_PARSER_KEYS_BY_SOURCE_FAMILY[
        GHG_PROTOCOL_SOURCE_FAMILY
    ]
...
def test_ghg_parser_adapter_descriptor_is_deterministic() -> None:
    first = describe_ghg_protocol_parser_adapter()
    second = describe_ghg_protocol_parser_adapter()
...
def test_ghg_parser_adapter_readiness_is_contract_only() -> None:
    descriptor = describe_ghg_protocol_parser_adapter()
...
def test_ghg_parser_adapter_descriptor_exposes_no_execution_methods() -> None:
    descriptor = describe_ghg_protocol_parser_adapter()
...
def test_ghg_parser_adapter_descriptor_uses_safe_passive_identifiers() -> None:
    descriptor = describe_ghg_protocol_parser_adapter()
...
def test_ghg_parser_adapter_contract_import_is_runtime_passive(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import builtins
    import os

    module_name = "carbonfactor_parser.parsers.ghg_protocol_adapter_contract"
    ...
    def guard_open(*args: object, **kwargs: object) -> object:
        ...
        raise AssertionError("GHG parser adapter contract import read a file")

    def guard_getenv(*args: object, **kwargs: object) -> object:
        getenv_calls.append(args)
        raise AssertionError("GHG parser adapter contract import read environment")
    ...
    assert hasattr(module, "describe_ghg_protocol_parser_adapter")
    assert open_calls == []
    assert getenv_calls == []
