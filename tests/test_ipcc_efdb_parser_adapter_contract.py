...
def test_ipcc_parser_adapter_descriptor_is_exact() -> None:
...
def test_ipcc_parser_adapter_supports_only_ipcc_efdb() -> None:
    descriptor = describe_ipcc_efdb_parser_adapter()

    assert descriptor.source_family == IPCC_EFDB_SOURCE_FAMILY
    assert descriptor.capability.source_family == IPCC_EFDB_SOURCE_FAMILY
    assert descriptor.source_family == "ipcc_efdb"
    assert descriptor.source_family not in {"ghg_protocol", "defra_desnz"}


def test_ipcc_parser_key_matches_selection_registry() -> None:
    descriptor = describe_ipcc_efdb_parser_adapter()
    registry = create_phase1_parser_selection_registry()
    registry_identity = next(
        identity
        for identity in registry.identities
        if identity.source_family == IPCC_EFDB_SOURCE_FAMILY
    )

    assert IPCC_EFDB_PARSER_KEY == PHASE1_PARSER_KEYS_BY_SOURCE_FAMILY[
        IPCC_EFDB_SOURCE_FAMILY
    ]
...
def test_ipcc_parser_adapter_descriptor_is_deterministic() -> None:
    first = describe_ipcc_efdb_parser_adapter()
    second = describe_ipcc_efdb_parser_adapter()
...
def test_ipcc_parser_adapter_descriptor_uses_safe_passive_identifiers() -> None:
    descriptor = describe_ipcc_efdb_parser_adapter()
...
def test_ipcc_parser_adapter_contract_import_is_runtime_passive(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import builtins
    import os

    module_name = "carbonfactor_parser.parsers.ipcc_efdb_adapter_contract"
    ...
    def guard_open(*args: object, **kwargs: object) -> object:
        ...
        raise AssertionError("IPCC EFDB parser adapter contract import read a file")

    def guard_getenv(*args: object, **kwargs: object) -> object:
        getenv_calls.append(args)
        raise AssertionError("IPCC EFDB parser adapter contract import read environment")
    ...
    assert hasattr(module, "describe_ipcc_efdb_parser_adapter")
    assert open_calls == []
    assert getenv_calls == []
