...
def test_cli_run_with_manifest_path_writes_local_manifest(
    ...
    exit_code = main(["run", "--manifest-path", str(manifest_path)])
    ...
    assert all(entry["status"] == "not_implemented" for entry in payload)
...
def test_cli_run_json_with_manifest_path_writes_local_manifest(
    capsys: pytest.CaptureFixture[str],
    tmp_path,
) -> None:
    manifest_path = tmp_path / "acquisition-manifest.json"
    ...
    exit_code = main(
        ["run", "--manifest-path", str(manifest_path), "--output-format", "json"]
    )
