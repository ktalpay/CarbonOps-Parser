...
def test_defra_discovery_request_is_deterministic_and_runtime_passive() -> None:
    first = create_defra_source_discovery_request()
    second = create_defra_source_discovery_request()

    assert first == second
    assert first == DEFRASourceDiscoveryRequest(
        source_family="defra_desnz",
        source_key="defra_desnz",
        discovery_reference_uri="discovery://defra_desnz/homepage",
        mode=DEFRASourceDiscoveryMode.RUNTIME_PASSIVE,
        allow_network=False,
    ...
    assert validate_defra_source_discovery_request(first).is_valid is True


def test_defra_discovery_result_declares_candidate_without_download() -> None:
    result = create_defra_source_discovery_result()

    assert result.status is DEFRASourceDiscoveryStatus.DECLARED
    ...
    assert validate_defra_source_discovery_result(result).is_valid is True

    candidate = result.candidates[0]
    assert candidate == DEFRASourceDocumentCandidate(
        source_family="defra_desnz",
        source_key="defra_desnz",
        candidate_id="defra_source_discovery_candidate_001_defra_desnz",
        title="DEFRA/DESNZ",
        reference_uri="discovery://defra_desnz/homepage",
        artifact_kind="discovery",
        status=DEFRASourceDiscoveryStatus.DECLARED,
...
def test_defra_discovery_boundary_is_defra_only() -> None:
    result = create_defra_source_discovery_result()

    assert tuple(candidate.source_key for candidate in result.candidates) == (
        "defra_desnz",
    )
    assert "ghg_protocol" not in result.candidate_ids
...
def test_invalid_request_fails_closed_with_no_candidates() -> None:
    ...
    result = create_defra_source_discovery_result(request)

    assert result.status is DEFRASourceDiscoveryStatus.INVALID
...
@pytest.mark.parametrize(
    ...
    (
        ("source_family", "DEFRA_SOURCE_DISCOVERY_MISSING_SOURCE_FAMILY"),
        ("source_key", "DEFRA_SOURCE_DISCOVERY_MISSING_SOURCE_KEY"),
        ("discovery_reference_uri", "DEFRA_SOURCE_DISCOVERY_MISSING_REFERENCE_URI"),
    ),
)
def test_request_required_fields_fail_closed(
    ...
    request = replace(create_defra_source_discovery_request(), **{field_name: " "})

    result = validate_defra_source_discovery_request(request)
...
def test_candidate_invalid_inputs_fail_closed() -> None:
    ...
    result = validate_defra_source_document_candidate(candidate)
...
def test_candidate_reference_is_not_fetched_or_downloaded(
    ...
    def fail_urlopen(*args: object, **kwargs: object) -> object:
        raise AssertionError("DEFRA discovery boundary must not fetch references")

    monkeypatch.setattr(urllib.request, "urlopen", fail_urlopen)

    assert validate_defra_source_document_candidate(candidate).is_valid is True
...
def test_validation_does_not_read_write_files_access_db_or_network(
    ...
    missing_reference = tmp_path / "defra-source.csv"

    def fail_side_effect(*args: object, **kwargs: object) -> object:
        raise AssertionError("DEFRA source discovery must use metadata only")
...
def test_result_validation_rejects_side_effect_flags() -> None:
    ...
    validation = validate_defra_source_discovery_result(result)
...
def test_defra_discovery_contract_dataclasses_are_immutable() -> None:
    request = create_defra_source_discovery_request()
    result = create_defra_source_discovery_result()
    candidate = result.candidates[0]
...
def test_validation_result_issue_shape_is_structured() -> None:
    candidate = replace(create_defra_source_discovery_result().candidates[0], title="")

    issue = validate_defra_source_document_candidate(candidate).issues[0]

    assert isinstance(issue, DEFRASourceDiscoveryIssue)
    assert issue.code == "DEFRA_SOURCE_DISCOVERY_CANDIDATE_MISSING_TITLE"
    assert issue.field_name == "title"
    assert issue.severity == "error"
    assert issue.message == "title must be a non-empty string."
    assert DEFRASourceDiscoveryValidationResult().is_valid is True
    assert DEFRASourceDiscoveryValidationResult(issues=(issue,)).is_valid is False


def test_defra_discovery_boundary_import_is_runtime_passive(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import builtins
    import os
    ...
    module_name = (
        "carbonfactor_parser.source_acquisition.defra_source_discovery_boundary"
    )
    ...
    def guard_open(*args: object, **kwargs: object) -> object:
        ...
        raise AssertionError("DEFRA discovery boundary import read a file")

    def guard_getenv(*args: object, **kwargs: object) -> object:
        getenv_calls.append(args)
        raise AssertionError("DEFRA discovery boundary import read environment")

    def guard_urlopen(*args: object, **kwargs: object) -> object:
        raise AssertionError("DEFRA discovery boundary import opened network")
    ...
    assert hasattr(module, "create_defra_source_discovery_result")
    assert open_calls == []
    assert getenv_calls == []
...
def test_defra_boundary_exports_through_contract_api() -> None:
    from carbonfactor_parser.source_acquisition import contract_api

    result = contract_api.create_defra_source_discovery_result()

    assert result.status is contract_api.DEFRASourceDiscoveryStatus.DECLARED
    assert result.candidate_count == 1
    assert contract_api.validate_defra_source_discovery_result(result).is_valid


def test_defra_boundary_does_not_regress_existing_ghg_boundaries(tmp_path) -> None:
