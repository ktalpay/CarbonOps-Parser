...
- CO-111A: Added a runtime-passive PostgreSQL Phase 1 schema bootstrap planner boundary that composes the existing schema catalog and DDL renderer into immutable deterministic planning metadata for required tables, CREATE TABLE statements, CREATE INDEX statements, statement ordering, target engine, phase marker, and no-execution scope while preserving no PostgreSQL connections, SQL execution, migrations, env/config loading, network behavior, filesystem writes, or new runtime dependencies.
- CO-110A: Added a committed plain-text PostgreSQL Phase 1 DDL renderer output contract snapshot covering shared/system and GHG/DEFRA/IPCC master/detail tables, keys, constraints, indexes, and CO-109A shortened identifiers, with focused snapshot equality, repeat-render determinism, shortened-name presence, and runtime-passive import coverage while preserving no DB connectivity, SQL execution, migrations, env loading, network behavior, or new runtime dependencies.
- CO-109A: Added PostgreSQL identifier length hardening to the runtime-passive DDL renderer with deterministic hash-suffixed shortening for overlength table, constraint, column, and index identifiers, focused coverage for generated FK/index names, repeated render determinism, identifier collision resistance, invalid identifier rejection, PostgreSQL 63-byte limits, and passive import behavior while preserving no DB connectivity, SQL execution, migrations, runtime dependencies, environment loading, or network behavior.
- CO-106A: Documented a PostgreSQL DDL naming/key/constraint/index strategy contract for Phase 1 planning, including shared/system and GHG/DEFRA/IPCC master-detail naming candidates, PK/FK/unique/index expectations, lifecycle and checksum/idempotency requirements, safety non-goals, and Python/.NET parity, while preserving documentation-only scope with no runtime code, migrations, or executable SQL.
- CO-105A: Documented the PostgreSQL Phase 1 startup/bootstrap boundary contract as documentation-only, including startup configuration inputs, PostgreSQL-only provider acceptance, pre-execution validation order, required shared/GHG/DEFRA/IPCC table-family checks, explicit fail-fast behavior when initialization is disabled and tables are missing, optional create-missing behavior only when explicitly enabled, strict no downloader/parser/data-seeding behavior during bootstrap, idempotency requirements, observability requirements, safety guardrails (no destructive schema operations and no credential/raw connection string logging), and Python/.NET conceptual parity requirements while preserving no runtime code, no migrations, no SQL execution, no database connection adapter implementation, and no scheduler/downloader/parser execution changes.
- CO-104A: Documented a documentation-first PostgreSQL Phase 1 schema contract defining shared ingestion/source metadata boundaries, source-family-specific GHG Protocol, DEFRA/DESNZ, and IPCC EFDB master/detail intent, required provenance/identity/version/hash/timestamp/parser/traceability/idempotency field intent, explicit non-goals, and future implementation gates while preserving unsupported runtime persistence, no SQL execution, no DB writes, no migrations or table creation behavior, no transaction behavior, no env/config/credential loading, and no production readiness claim.
...
- CO-102A: Added shared runtime-passive ingestion contract baselines (`SourceType`, `SourceDocument`, `SourceAcquisitionResult`, `ParsedFactorRecord`, `IngestionRun`, `IngestionStatus`, `PersistenceBootstrapResult`) with deterministic import/immutability/value coverage tests and boundary documentation while preserving no HTTP, no DB connections, no parser execution, no scheduler behavior, no credential loading, no environment loading, no CLI behavior changes, and no runtime side effects at import time.
