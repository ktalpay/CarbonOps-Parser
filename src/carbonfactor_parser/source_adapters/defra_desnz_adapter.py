"""DEFRA/DESNZ source adapter skeleton for local fixture discovery."""
...
class DefraDesnzSourceAdapter:
    """Discover local artificial DEFRA/DESNZ fixture files."""

    directory_path: str | Path
    source_key: str = "defra_desnz"
    allowed_extensions: Iterable[str] | str | None = (".csv", ".json")
    allowed_name_prefixes: Iterable[str] | str | None = "defra_desnz_"
    ...
    def discover(self) -> AdapterDiscoveryResult:
        if not self.directory_path.is_dir():
            return AdapterDiscoveryResult(
                documents=(),
                warnings=(
                    f"DEFRA/DESNZ source directory not found: {self.directory_path}",
                ),
            )
