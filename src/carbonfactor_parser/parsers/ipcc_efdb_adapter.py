...
class IpccEfdbParserAdapter:
    """IPCC EFDB parser adapter for already-loaded content parsing."""
    ...
    def can_parse(self, parser_input: ParserInputContract) -> bool:
        """Return True for matching IPCC EFDB parser input metadata."""
    ...
    def parse(self, parser_input: ParserInputContract) -> ParserExecutionResult:
        ...
        return create_parser_execution_result(
            ...
            issues=(
                ParserExecutionIssue(
                    code="IPCC_EFDB_PARSER_REQUIRES_LOADED_CONTENT",
                    message=(
                        "IPCC EFDB parser adapter requires caller-provided "
                        "content via parse_content."
                    ),
                    severity=ParserExecutionIssueSeverity.WARNING,
                    location="ipcc_efdb_parser_adapter",
                ),
            ),
            parser_metadata={
    ...
    def parse_content(
        ...
        """Parse already-loaded IPCC EFDB content."""

        return parse_ipcc_efdb_file_content(content_input)
