...
The parser package exposes `ParserInputContract`, `create_parser_input_contract()`, and `validate_parser_input_contract()` for future parser adapter input handoff. The contract carries acquisition metadata only; it does not contain parser output, normalization output, or database persistence fields.
