...
The parser package exposes `ParserInputContract`, `create_parser_input_contract()`, `validate_parser_input_contract()`, the `ParserAdapter` protocol, parser adapter registry helpers, and parser execution planning helpers for future parser adapter input handoff. The contract carries acquisition metadata only; it does not contain parser output, normalization output, or database persistence fields.
